...
export function TreatmentCard({ treatment, selected, onSelect }: TreatmentCardProps) {
  ...
  return (
    <Card
      className={cn(
        "treatment-card p-3 sm:p-4 md:p-5 cursor-pointer border-2 transition-all duration-300 transform touch-friendly",
        "hover:scale-105 active:scale-[0.98] hover:shadow-xl min-h-[100px] sm:min-h-[120px] md:min-h-[140px]",
    ...
    >
      <div className="text-center space-y-2 sm:space-y-3 h-full flex flex-col justify-center">
        <div className={cn(
          "w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center mx-auto shadow-lg border-2 transition-all duration-300",
          selected 
          ...
          {treatment.category === 'emergency' && (
            <svg viewBox="0 0 24 24" className={cn("w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6", selected ? "text-white" : "text-red-600")} fill="currentColor">
          ...
          {treatment.category === 'urgent' && (
            <svg viewBox="0 0 24 24" className={cn("w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6", selected ? "text-white" : "text-orange-600")} fill="currentColor">
          ...
          {treatment.category === 'routine' && (
            <svg viewBox="0 0 24 24" className={cn("w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6", selected ? "text-white" : "text-green-600")} fill="currentColor">
              <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/>
            </svg>
          )}
          {treatment.category === 'cosmetic' && (
            <svg viewBox="0 0 24 24" className={cn("w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6", selected ? "text-white" : "text-blue-600")} fill="currentColor">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
            </svg>
          )}
        </div>
        <div className="px-1">
          <h4 className={cn("font-bold text-xs sm:text-sm md:text-base leading-tight", selected ? 'text-primary' : 'text-gray-900')}>
            {treatment.name}
          </h4>
